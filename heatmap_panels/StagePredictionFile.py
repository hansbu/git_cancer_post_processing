...
class StagePredictionFile(XYLabelFile):
    ...
    def get_stage_prediction(self):
        ...
        self.benign, self.pred = self.get_benign_tumor_adjusted(self.benign, self.g3+self.g45+self.benign, len(indexes))
    ...
    def get_benign_tumor_adjusted(self, benign, sum_probabilities, num_classes):
        ...
        for i, p in enumerate(benign.reshape(-1)):
            if sum_probabilities[i] > 0:
                benign_adjusted[i] = benign[i] * (num_classes - 1) / (sum_probabilities[i] + benign[i] * (num_classes - 2))
