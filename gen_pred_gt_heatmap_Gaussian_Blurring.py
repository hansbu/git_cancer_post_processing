...
for count, mask_fn in enumerate(mask_files):
    ...
    temp = (iml*255).astype(np.uint8)
    temp = iml.copy()
    #smooth = cv2.GaussianBlur(temp, (3, 3), 0)
    smooth5 = cv2.GaussianBlur(temp, (5, 5), 0)
#    smooth7 = cv2.GaussianBlur(temp, (7, 7), 0)


    # temp = cv2.applyColorMap(temp, cv2.COLORMAP_JET)
    # smooth3 = cv2.applyColorMap(smooth3, cv2.COLORMAP_JET)
    # smooth5 = cv2.applyColorMap(smooth5, cv2.COLORMAP_JET)
    # smooth7 = cv2.applyColorMap(smooth7, cv2.COLORMAP_JET)
    # dum = (np.ones((temp.shape[0], 3, 3))*255).astype(np.uint8)
    # temp = np.hstack((temp, dum, smooth3, dum, smooth5, dum, smooth7))
    # print('size of heatmap: ', temp.shape)
    # cv2.imshow('img', temp)
    ...
    for ind, sub_fol in enumerate(sub_fols):
...
pool = mp.Pool(processes=len(sub_fols))
results = [pool.apply(compute_dice, args=(os.path.join(out_fol, fol),)) for fol in sub_fols]
print(results)

#
# for fol in sub_fols:
#     compute_dice(os.path.join(out_fol, fol))
